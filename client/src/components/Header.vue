        ...
        >
          <div class="user-avatar flex p-1 cursor-pointer rounded-md">
            ...
            <div class="text-left lg:block md:block hidden">
              <h2 class="dark:text-white text-gray-800" style="margin-top: 1vh;">Hi, {{ dataUser.name }}</h2>
            </div>
        ...
        <transition name="fade">
          ...
          >
            <div class="py-3 px-4 text-sm text-gray-900 dark:text-gray-200">
              <div>Logged As</div>
              <div class="font-medium truncate">{{ dataUser.name }}</div>
            </div>
            <ul
...
import { useCounterStore } from "@/store/counter";
  export default {
    ...
    computed:{
      ...mapState(useCounterStore,['dataUser'])
    },
    mounted() {
